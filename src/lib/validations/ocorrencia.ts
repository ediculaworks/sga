...
/**
 * Schema BASE - campos do formulário sem refinements de objeto
 * (para poder fazer extend depois)
 */
...
/**
 * Schema para FORMULÁRIO (sem tipo_ambulancia, COM validações)
 */
export const criarOcorrenciaFormSchema = baseFieldsSchema
  .refine(
    (data) => {
  ...
  .refine(
    (data) => {
      // Validação: horario_termino deve ser depois de horario_chegada_local
...
/**
 * Schema COMPLETO (com tipo_ambulancia)
 */
export const criarOcorrenciaSchema = baseFieldsSchema
  .extend({
  ...
  .refine(
    (data) => {
      // Validação: valor_medico é obrigatório se tipo_ambulancia é UTI
